    ...
    >
      ...
      <div className="flex justify-between items-center mt-1 pt-3 border-t border-white/5">
        <span 
          ...
          className={`text-[10px] font-bold font-mono uppercase tracking-widest px-2.5 py-1 rounded-full border ${(myRole === 'OWNER' || myRole === 'ADMIN') ? 'cursor-pointer hover:opacity-80 transition-opacity' : ''} ${task.priority === 'URGENT' ? 'border-red-500 text-red-100 bg-red-600 shadow-[0_0_15px_rgba(239,68,68,0.8)]' : task.priority === 'HIGH' ? 'border-red-500/50 text-red-400 bg-red-500/10' : task.priority === 'MEDIUM' ? 'border-yellow-500/50 text-yellow-400 bg-yellow-500/10' : 'border-green-500/50 text-green-400 bg-green-500/10'}`}
        >
          {task.priority}
        </span>
      </div>
