...
const ProjectBoard = () => {
        ...
        if (isActiveTask && isOverTask) {
            ...
            if (localTasks[activeIndex].status !== localTasks[overIndex].status) {
                setLocalTasks((prevTasks) => {
                 ...
                 setLocalTasks((prevTasks) => arrayMove(prevTasks, activeIndex, overIndex));
            }
        }
    ...
    const handleCreateTask = async (e) => {
        e.preventDefault();
        await createTask({ projectId, title: newTask.title, priority: newTask.priority });
        ...
        <main className="flex-1 overflow-auto pt-8 px-10 pb-10 flex flex-col relative custom-scrollbar w-full h-full">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
                    <div className="flex items-center space-x-4 mb-6 md:mb-0">
                        ...
                        <div>
                            <h1 className="text-4xl md:text-5xl font-serif text-white tracking-tight">{currentProject?.name || "Project"}</h1>
                ...
                <div className="w-full flex space-x-6 items-start mt-8 relative z-10">
                        ...
                        >
                            {COLUMNS.map(column => {
                                ...
                                columnTasks.sort((a, b) => priorityWeight[b.priority] - priorityWeight[a.priority]);
                                
                                const taskIds = columnTasks.map(t => t._id);

                                return (
                                    <ColumnDroppable key={column.id} column={column} taskCount={columnTasks.length}>
                                        <SortableContext items={taskIds} strategy={verticalListSortingStrategy}>
                                            <div className="p-4 space-y-3 pb-8 min-h-[150px]">
                                                {columnTasks.map(task => (
                                                    <SortableTask key={task._id} task={task} myRole={currentProject?.myRole} updateTaskPriority={updateTaskPriority} onOpenDetails={setSelectedTaskId} />
                                                ))}
                                            </div>
                                        </SortableContext>
                                    </ColumnDroppable>
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
                            ...
                            >
                                ...
                                <form onSubmit={handleCreateTask} className="space-y-6">
                                    <div>
                                        <label className="block text-sm font-bold font-mono text-neutral-500 mb-2 uppercase tracking-[0.2em]">Task Title</label>
                                        <input type="text" required value={newTask.title} onChange={e => setNewTask({...newTask, title: e.target.value})} className="w-full bg-white/5 border border-white/10 px-6 py-4 rounded-full text-base text-white focus:outline-none focus:border-[#3b82f6] transition-colors" placeholder="e.g. Design Login Page" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold font-mono text-neutral-500 mb-4 uppercase tracking-[0.2em]">Priority</label>
                                            ...
                                            {['LOW', 'MEDIUM', 'HIGH', 'URGENT'].map(p => (
                                                <button
                                                    ...
                                                    className={`flex-1 py-3 rounded-full font-mono text-[10px] sm:text-xs font-bold uppercase tracking-widest border transition-all duration-300 ${
                                                        newTask.priority === p
                                                        ? p === 'URGENT' ? 'border-red-500 text-red-100 bg-red-600 shadow-[0_0_15px_rgba(239,68,68,0.8)]' :
                                                          p === 'HIGH' ? 'border-red-500 text-red-400 bg-red-500/10 shadow-[0_0_10px_rgba(239,68,68,0.3)]' :
                                                          p === 'MEDIUM' ? 'border-yellow-500 text-yellow-400 bg-yellow-500/10 shadow-[0_0_10px_rgba(234,179,8,0.3)]' :
                                    ...
                                    <div className="flex justify-end space-x-4 pt-6 border-t border-white/5">
                                        <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-4 text-sm font-bold font-mono text-neutral-400 hover:text-white uppercase tracking-widest transition-colors">Cancel</button>
                                        <motion.button 
                                            whileHover={{ scale: 1.02 }}
                                            whileTap={{ scale: 0.98 }}
                                            type="submit" className="bg-[#3b82f6] hover:bg-[#2563eb] text-white px-8 py-4 font-sans text-base tracking-widest uppercase font-semibold flex items-center transition-colors shadow-[0_0_20px_rgba(255,90,0,0.3)] rounded-full"
                                        >
                                            Create
                                        </motion.button>
                                    </div>
